...
class TaskManager(object):
    ...
    def _load_history(self):
        ...
        if "XDG_CONFIG_HOME" in os.environ:
            xdg = os.environ["XDG_CONFIG_HOME"]
        local = os.path.join(xdg, "asynctask")
        if not os.path.exists(local):
            try:
                os.makedirs(local)
            except:
                pass
        self.history_file = os.path.join(local, "history.json")
    ...
    def _get_history(self, task, varname):
        key = "%s:%s" % (task, varname)
        return self.history.get(key, [])

    def _add_history(self, task, varname, text):
        if not text:
            return
        key = "%s:%s" % (task, varname)
    ...
    def _handle_input(self, varname):
        ...
        if "-" in self.config.shadow:
            ...
            if name in shadow:
                return shadow[name]
        history = self._get_history(self._current_task, name)
        if "," not in tail:
        ...
        if not text:
            return None
        self._add_history(self._current_task, name, text)
        return text
